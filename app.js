...
const CONFIG = {
  ...
  pegel: {
    // Statt eines fest geratenen Stationsnamens suchen wir die Station
    // dynamisch über die Wasserstraße "SPREE" und den Namen "Köpenick".
    searchUrl: "https://www.pegelonline.wsv.de/webservices/rest-api/v2/stations.json?waters=SPREE&prettyprint=false"
  },
  einsaetze: {
...
async function loadPegel(){
  try{
    // Schritt 1: Alle Spree-Stationen holen und die Köpenick-Station darin finden.
    // So verlassen wir uns nicht auf einen geratenen Stationsnamen.
    const stationsRes = await fetch(CONFIG.pegel.searchUrl);
    if(!stationsRes.ok) throw new Error("Stationssuche: HTTP " + stationsRes.status);
    const stations = await stationsRes.json();
    const station = stations.find(s => /köpenick|koepenick/i.test(s.longname || s.shortname || ""));
    if(!station) throw new Error("Station Köpenick nicht in der Stationsliste gefunden");

    // Schritt 2: Messwerte der letzten 30 Tage über die UUID der Station abrufen.
    const measUrl = `https://www.pegelonline.wsv.de/webservices/rest-api/v2/stations/${station.uuid}/W/measurements.json?start=P30D`;
    const res = await fetch(measUrl);
    if(!res.ok) throw new Error("Messwerte: HTTP " + res.status);
    const rows = await res.json();
    if(!rows || !rows.length) throw new Error("Keine Messwerte vorhanden");

    state.pegel = { rows, station, fetchedAt: new Date() };
    renderPegel();
  }catch(err){
...
function renderPegel(){
  ...
  const { rows, station, fetchedAt } = state.pegel;
  if(station){
    document.querySelector(".pegel-station").textContent =
      `${station.longname || station.shortname}` + (station.water ? `, ${station.water.longname}` : "");
  }
...
/* ============================ EINSÄTZE ============================ */
// Durchsucht das GitHub-Repo rekursiv (bis zu 3 Ebenen tief) nach CSV-Dateien.
async function findCsvFiles(url, depth){
  const res = await fetch(url);
  if(!res.ok){
    if(res.status === 403) throw new Error("GitHub-API-Limit erreicht (403) – bitte später erneut versuchen");
    throw new Error("HTTP " + res.status + " bei " + url);
  }
  const entries = await res.json();
  if(!Array.isArray(entries)) throw new Error("Unerwartete GitHub-Antwort: " + JSON.stringify(entries).slice(0,120));

  let found = entries.filter(e => e.type === "file" && e.name.toLowerCase().endsWith(".csv"));
  if(found.length === 0 && depth > 0){
    const dirs = entries.filter(e => e.type === "dir");
    for(const dir of dirs){
      const sub = await findCsvFiles(dir.url, depth - 1);
      found = found.concat(sub);
      if(found.length > 0) break; // erste Fundstelle reicht uns
    }
  }
  return found;
}
async function loadEinsaetze(){
  try{
    const csvFiles = await findCsvFiles(CONFIG.einsaetze.apiRoot, 3);
    if(!csvFiles.length) throw new Error("Keine CSV-Datei im Open-Data-Repo gefunden (auch nicht in Unterordnern)");
    // Bevorzugt eine Datei mit "kiez" oder "monat" im Namen, sonst die erste gefundene
    const csvFile = csvFiles.find(f => /kiez|monat|einsaetze|einsätze/i.test(f.name)) || csvFiles[0];
